...
//
// It runs in the service worker rather than the page for two reasons:
...
// The palette is sampled from a downscale of the source: a few thousand pixels
// is plenty to pick tile colours from, and it keeps peak memory flat regardless
// of how large the original image was.
const SAMPLE_DIM = 256;

// Used when the source cannot be decoded at all and the caller told us how much
// space the asset occupies on screen.
 ...
 *
 * Always resolves. If the source cannot be fetched or decoded we still return a
 * replacement -- one synthesized from the URL -- because the contract this
 * extension makes with the user is that *every* asset gets replaced, not every
 * asset we happen to be able to read.
 *
 ...
 */
export function replacementFor(request) {
...
export function clearReplacementCache() {
  cache.clear();
}
 ...
 * gathers colours that belong to the original while scrambling where they sit,
 * so the mosaic reads as "same palette, no content". Sampling in place -- or
 * averaging per tile -- would reproduce a recognizable pixelation of the source,
 * which is exactly what this extension exists to prevent.
 */
...
// `FileReader` and `URL.createObjectURL` are both unavailable in a service
// worker, so base64 is assembled by hand.
